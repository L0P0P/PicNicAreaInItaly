...
app.get("/rawdata", (req, res) => {
  console.log(
    "Richiesta dei dati puri, l'operazione potrebbe richiedere alcuni minuti..."
  );
  res.send(parser);
});
...
function validateArea(area) {
  const schema = Joi.object({
    ...
    clatitudine: Joi.number().required(),
    clongitudine: Joi.number().required(),
  });
...
app.delete("/removing", (req, res) => {
  const area = parser.find(
    (a) =>
      a.clongitudine === req.body.clongitudine &&
...
app.get("/research", (req, res) => {
  const area = parser.find(
    (a) =>
      a.clongitudine === req.query.clongitudine &&
...
app.put("/modification", (req, res) => {
  ...
  if (parser[parser.indexOf(area)].cnome === req.body.cnome)
